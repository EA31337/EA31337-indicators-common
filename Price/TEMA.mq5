...
/**
 * @file
 * Implements Triple Exponential Moving Average (TEMA) indicator.
 */

// Defines.
#define INDI_FULL_NAME "Triple Exponential Moving Average"
#define INDI_SHORT_NAME "TEMA"
...
void OnInit() {
  ...
  IndiTEMAParams _indi_params(::InpMAPeriod, ::InpMAShift, ::InpMAAppliedPrice,
                              ::InpShift_);
  indi = new Indi_TEMA(_indi_params, ::InpSourceType);
  // Name for labels.
...
int OnCalculate(const int rates_total, const int prev_calculated,
  ...
  int i, start;
  if (rates_total < 2 * InpMAPeriod - 1) {
    return (0);
  }
